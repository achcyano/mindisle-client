...
final doctorAuthApiProvider = Provider<DoctorAuthApi>((ref) {
  return DoctorAuthApi(ref.watch(appDioProvider));
});

final doctorAuthRepositoryProvider = Provider<DoctorAuthRepository>((ref) {
  return DoctorAuthRepositoryImpl(
    api: ref.watch(doctorAuthApiProvider),
    sessionStore: ref.watch(sessionStoreProvider),
  );
});

final sendDoctorSmsCodeUseCaseProvider = Provider<SendDoctorSmsCodeUseCase>((ref) {
...
final doctorLoginPasswordUseCaseProvider = Provider<DoctorLoginPasswordUseCase>((ref) {
  return DoctorLoginPasswordUseCase(ref.watch(doctorAuthRepositoryProvider));
});

final doctorRefreshTokenUseCaseProvider = Provider<DoctorRefreshTokenUseCase>((ref) {
  return DoctorRefreshTokenUseCase(ref.watch(doctorAuthRepositoryProvider));
});

final doctorResetPasswordUseCaseProvider = Provider<DoctorResetPasswordUseCase>((ref) {
  return DoctorResetPasswordUseCase(ref.watch(doctorAuthRepositoryProvider));
});

final doctorChangePasswordUseCaseProvider = Provider<DoctorChangePasswordUseCase>((ref) {
  return DoctorChangePasswordUseCase(ref.watch(doctorAuthRepositoryProvider));
});
